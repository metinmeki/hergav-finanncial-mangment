...
<head>
    <meta charset="UTF-8">
    <title>وصل رقم {{ $tx->receipt_number ? str_pad($tx->receipt_number, 6, '0', STR_PAD_LEFT) : $tx->reference_no }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .divider { border: none; border-top: 2px solid {{ $tx->type === 'deposit' ? '#16a34a' : '#dc2626' }}; margin: 8px 0; }
...
<body>
    ...
    <div class="receipt">
        <div class="header">
            ...
            @if($tx->type === 'deposit')
                <span class="badge" style="background: #f0fdf4; border: 1px solid #86efac; color: #16a34a;">وصل إيداع</span>
            @else
                <span class="badge" style="background: #fef2f2; border: 1px solid #fecaca; color: #dc2626;">وصل سحب</span>
            @endif
        </div>
        <hr class="divider">
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                ...
                <span class="info-value" dir="ltr" style="color: #1a3c5e; font-size: 10px;">{{ $tx->reference_no }}</span>
            </div>
            <div class="info-item">
                <span class="info-label">{{ $tx->type === 'deposit' ? 'اسم المرسل' : 'اسم المستلم' }}</span>
                <span class="info-value">{{ $tx->sender_name ?? '—' }}</span>
            </div>
            <div class="info-item">
                <span class="info-label">{{ $tx->type === 'deposit' ? 'هاتف المرسل' : 'هاتف المستلم' }}</span>
        ...
        <table class="tx-table">
            <thead>
                <tr style="background: {{ $tx->type === 'deposit' ? '#16a34a' : '#dc2626' }};">
                    <th>البيان</th>
                    <th>العملة</th>
                    <th>المبلغ</th>
                </tr>
            ...
            <tbody>
                ...
                <tr style="background: {{ $tx->type === 'deposit' ? '#f0fdf4' : '#fef2f2' }};">
                    <td style="color: {{ $tx->type === 'deposit' ? '#16a34a' : '#dc2626' }}; font-weight: 600;">
                        {{ $tx->type === 'deposit' ? 'المبلغ المودع' : 'المبلغ المسحوب' }}
                    </td>
                    <td style="color: {{ $tx->type === 'deposit' ? '#16a34a' : '#dc2626' }}; font-weight: 600;">{{ $tx->currency->code }}</td>
                    <td style="color: {{ $tx->type === 'deposit' ? '#16a34a' : '#dc2626' }}; font-weight: 700; font-size: 14px;" dir="ltr">
{{ number_format($tx->amount, $tx->currency->code === 'IQD' ? 0 : 2) }}                    </td>
                </tr>
                <tr style="border-bottom: 2px solid {{ $tx->type === 'deposit' ? '#16a34a' : '#dc2626' }};">
